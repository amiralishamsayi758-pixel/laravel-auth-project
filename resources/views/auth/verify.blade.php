        ...
        <div class="w-full max-w-lg">
            ...
            <div class="w-full rounded-lg border border-white/75 bg-white/70 p-6 shadow-[0_22px_60px_-28px_rgba(15,23,42,.35)] backdrop-blur-xl dark:border-white/[.08] dark:bg-[#111b20]/85 sm:p-9 lg:p-10">
                ...
                <form action="#" method="post" class="space-y-6">
                    @csrf
                    <fieldset><legend class="sr-only">چهار رقم کد تأیید</legend><div data-code-inputs class="flex w-full justify-center gap-2 sm:gap-4" dir="ltr">@foreach (range(1, 4) as $digit)<input class="code-input" type="text" maxlength="1" inputmode="numeric" autocomplete="{{ $digit === 1 ? 'one-time-code' : 'off' }}" aria-label="رقم {{ $digit }} کد تأیید">@endforeach</div></fieldset>
                    <button class="primary-button" type="submit">تأیید کد</button>
                </form>
                <form action="#" method="post" class="mt-6 border-t border-slate-200/70 pt-5 dark:border-white/10">@csrf<button type="submit" class="w-full rounded-md border border-teal-600/25 bg-white/40 px-4 py-3 text-sm font-semibold text-brand transition hover:bg-teal-50 dark:border-teal-300/20 dark:bg-white/[.04] dark:text-teal-300">ارسال مجدد کد</button></form>
            </div>
        </div>
    </section>
</main>
